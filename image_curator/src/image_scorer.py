...
def get_technical_score(image_array, is_original_image=True):
    ...
    try:
        ...
        blur_score = np.clip(laplacian_var / 25, 0, 10)
        ...
        contrast_score = np.clip(contrast / 25, 0, 10)
        ...
        noise_score = np.clip(10 - (noise_level / 2), 0, 10)

        height, width = image_array.shape[:2]
        pixels = height * width
        resolution_score = np.clip(pixels / 100000, 0, 10)
        ...
        if is_original_image:
            final_tech_score = min(final_tech_score * 1.2, 10.0)
    ...
    if clip_model is not None and clip_processor is not None and device is not None:
        try:
            ...
            with torch.no_grad():
                ...
                score += (positive_score - negative_score) * 4
        except Exception as e:
