...
class EventAnalytics:
    ...
    def get_participant_stats(self, participant_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'ticket_type' in participant_df.columns:
            ...
            if 'Student' in stats['ticket_type_dist']:
                stats['student_count'] = stats['ticket_type_dist']['Student']
                stats['student_percentage'] = (stats['student_count'] / len(participant_df) * 100)

        # Analyze company size if column exists
        if 'company_size' in participant_df.columns:
            ...
            stats['organization_dist'] = {
                size: company_sizes.get(size, 0) for size in company_size_order if size in company_sizes.index
            }

        # Analyze job title if column exists
        if 'job_title' in participant_df.columns:
            # You can add more detailed role analysis here if needed
            stats['role_dist'] = participant_df['job_title'].value_counts().head(10).to_dict()

        # Analyze registration timeline if column exists
        if 'registration_date' in participant_df.columns:
            participant_df['registration_date'] = pd.to_datetime(participant_df['registration_date'], errors='coerce')
            daily_reg = participant_df.dropna(subset=['registration_date']).groupby(participant_df['registration_date'].dt.date).size()
            
            if not daily_reg.empty:
                stats['registration_timeline'] = {str(k): v for k, v in daily_reg.to_dict().items()}
                peak_day = daily_reg.idxmax()
                stats['peak_registration_day'] = {'date': str(peak_day), 'count': daily_reg.max()}
                stats['first_registration'] = str(daily_reg.index.min())
